...
map::map()
{
	...
	set(23,26,bigdot);
       ...
       /*setup environment, noncanonical, no echo */
	...
	std::cout<<CLEAR<<CHIDE<<SIZE;
     #ifdef COLOR  
	printf("\033[0;40m");    //set background color to black
     #endif 
}
	...
	for (int i(0); i < 31; i++)
	  {
		for (int j(0); j < 28; j++)
          	{
	#ifdef COLOR
		 if ((screen[i][j] == "|") || (screen[i][j] == over)
			|| (screen[i][j] == under) || (screen[i][j] == "_"))
			{
			   printf("\033[38;5;21m");
			}
	#endif
	...
	for (int i(0); i < 31; i++)
	  {
		...
		else if ((screen[i][j] == "|") || (screen[i][j] == over)
			|| (screen[i][j] == under) || (screen[i][j] == "_"))
			{
			   printf("\033[38;5;21m");
			}
...
pacman::pacman(map& m)
{
	 r = 23;  //initial position
	 c = 14;  //initial position
}

//accessors
Row pacman::position1()
{
	return r;
}

Col pacman::position2()
{
	return c;
}
...
{
	//check ghost and player position
	if(m.getscreen(position1(), position2()) == ghost)
	{
		lose = 1;
		return;
	}
	...
	{ 
          ...
          if ( m.isValidPosition(position1()-1,position2()) )	
	   { 
	      m.points(position1()-1,position2(), score);
	      m.set(position1(), position2()," ");
	      m.set(position1()-1,position2(), Upman);
	      r--;	
   	   } 		
	}
	...
	else if (n == 'k') //move down
	{
	  ...
	  if ( m.isValidPosition(position1()+1,position2()) )	
	       ...
	       m.points(position1()+1,position2(), score);
               m.set(position1(), position2()," ");
	       m.set(position1()+1, position2(), Dpman);
	       r++;
            }
	}
	...
	else if (n == 'j')  //move left
	{
	  ...
	  if ( m.isValidPosition(position1(),position2()-1) )
	    {
	       m.points(position1(), position2()-1, score);
	       m.set(position1(), position2(), " ");
	       m.set(position1(), position2()-1, Lpman);
	       c--;
            }
	}
	...
	else if (n == 'l')   //move right
	{
	  ...
	  if ( m.isValidPosition(position1(),position2()+1) )
	  {
	       m.points(position1(), position2()+1, score);
	       m.set(position1(), position2(), " ");
	       m.set(position1(), position2()+1, Rpman);
	       c++;
	  }
	}
	
	//Tunnel Fix
        if (position1() == 14 && position2() == 28)
	{
	  m.set(position1(), position2(), " ");
	  m.set(position1()-1, position2(), Rpman); 
	  c=0;
	}
	if (position1() == 14  && position2() == -1)
	{
	   m.set(position1(), position2(), " ");
           m.set(position1()+1, position2(), Lpman);
           r=14; c=27;
	}
...
{
        if(start == 1)  //ghost initial position
        {
                gy = 14;
                gx = 14;
        }
...
void enemy::move(map& m, pacman& p, int& lose)
{
   // Move out of central ghost box 
	...
	{
	m.set(enemyy(), enemyx(), under);
        m.set(enemyy()-1, enemyx(), ghost);
        gy = 11;
        }
    //what to do outside of box 
    else
   {
   ...
   float currentdist = sqrt(pow(abs(enemyy() - p.position1()), 2) + pow(abs(enemyx() - p.position2()),2)); 
   ...
   if(m.isValidPosition(enemyy(), enemyx()+1))
   {
	...
	//check ghost and player position
	if  (p.position1() == gy && (p.position2() == gx))
	{
	   lose = 1;
	}
